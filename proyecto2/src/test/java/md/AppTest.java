...
public class AppTest 
{
	...
	public void processedTextTest(){
		boolean isNonWordChar = false;
		String processedMsg = io.processString(message);
		int len = processedMsg.length();
		for(int i=0; i < len;i++){
			char c = processedMsg.charAt(i);
			if(c < 65 | c > 90){//Entre 65 y 90
				isNonWordChar = true;
			}  
		}
		assertFalse(isNonWordChar);
	}
}
